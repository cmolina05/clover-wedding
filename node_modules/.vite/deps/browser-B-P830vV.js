...
var Engine = class {
	...
	async load(params) {
		...
		const { Container } = await import("./Container-CdocublJ.js"), id = params.id ?? domSourceElement?.id ?? `tsparticles${Math.floor(getRandom() * 1e4).toString()}`, { index, url } = params, options = url ? await getDataFromUrl({
			fallback: params.options,
			url,
			index
		}) : params.options, currentOptions = itemFromSingleOrMultiple(options, index), { items } = this, oldIndex = items.findIndex((v) => v.id.description === id), newItem = new Container({
